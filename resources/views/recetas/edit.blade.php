    ...
    <x-slot name="header">
        <nav class="font-semibold text-xl text-gray-800 leading-tight" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li>
                    <a href="#" class="text-gray-500" aria-current="page">Editar receta</a>
                </li>
            </ol>
        </nav>
    </x-slot>
                            ...
                            @else
                                <x-select id="paciente_id" name="paciente_id" required>
                                    ...
                                    @foreach ($pacientes as $paciente)
                                        <option value="{{$paciente->id}}" @if ($receta->paciente_id == $paciente->id) selected @endif>{{$paciente->user->name}} ({{$paciente->nuhsa}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                <td class="py-3 px-6 text-left whitespace-nowrap">
                                    <div class="flex items-center">
                                        <span class="font-medium">{{$medicamento->nombre}} ({{$medicamento->dosis}} {{__('mg')}})</span>
                                    </div>
                                </td>
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center">
                                        ...
                                        <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                            <form id="detach-form-{{$receta->id}}-{{$medicamento->id}}" method="POST" action="{{ route('recetas.detachMedicamento', [$receta->id, $medicamento->id]) }}">
                                                @csrf
                                                @method('delete')
                                                <a class="cursor-pointer" onclick="getElementById('detach-form-{{$receta->id}}-{{$medicamento->id}}').submit();">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('recetas.attachMedicamento', [$receta->id]) }}">
                        @csrf
                        ...
                        <div class="mt-4">
                            ...
                            <x-select id="medicamento_id" name="medicamento_id" required>
                                ...
                                @foreach ($medicamentos as $medicamento)
                                    <option value="{{$medicamento->id}}" @if (old('medicamento_id') == $medicamento->id) selected @endif>{{$medicamento->nombre}} ({{$medicamento->dosis}} {{__('mg.')}})</option>
                                @endforeach
                            </x-select>
                        </div>
